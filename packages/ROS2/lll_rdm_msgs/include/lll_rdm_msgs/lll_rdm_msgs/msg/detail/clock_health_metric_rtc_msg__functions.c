  ...
  lll_rdm_msgs__msg__ClockHealthMetricRtcMsg__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    lll_rdm_msgs__msg__ClockHealthMetricRtcMsg * data =
      (lll_rdm_msgs__msg__ClockHealthMetricRtcMsg *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!lll_rdm_msgs__msg__ClockHealthMetricRtcMsg__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          lll_rdm_msgs__msg__ClockHealthMetricRtcMsg__fini(&output->data[i]);
        }
        return false;
      }
    }
